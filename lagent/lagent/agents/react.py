...
class ReActProtocol:
    ...
    def parse(
        ...
        # 使用正则表达式找到第一个感叹号的位置
        end_index = re.search(r'！', message)

        # 如果找到了感叹号，截取感叹号及其之前的部分作为 thought
        if end_index:
            thought = message[:end_index.end()]
        else:
            # 如果没有找到感叹号，继续查找第一个"谨慎考虑"
            caution_match = re.search(r'谨慎考虑。', message)
            if caution_match:
                thought = message[:caution_match.end()]
            else:
                # 如果都没有找到，直接将 message 的内容赋值给 thought
                thought = message
        ...
        return thought, action_input.strip().strip('"')
...
class ReAct(BaseAgent):
    ...
    def chat(self, message: str) -> AgentReturn:
        ...
        for turn in range(self.max_turn):
            ...
            thought, action_input = self._protocol.parse(
